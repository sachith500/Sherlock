...
def get_split_info(args):
    ...
    if args['group'] == 'type':
        ...
        train_labels = [0 if 'benign' in file.split('malnet-images/')[1].rsplit('/', 2)[0] else 1 for file in
                        files_train]
    ...
    print('Number of train samples: {}, val samples: {}, test samples: {}'.format(len(files_train), len(files_val),
                                                                                  len(files_test)))
...
def copy_images(args):
        ...
        for src_path in files_train:
            ...
            if args['group'] == 'binary':
                if 'benign' not in dst_path:
                    dst_path = dst_path.split('train/')[0] + 'train/malicious/' + \
                               dst_path.split('train/')[1].split('/')[2]
                else:
                    dst_path = dst_path.split('train/')[0] + 'train/benign/' + dst_path.split('train/')[1].split('/')[2]

            elif args['group'] == 'family':
                dst_path = dst_path.split('train/')[0] + 'train/' + '/'.join(
                    dst_path.split('train/')[1].split('/')[1:3])
    ...
    if args['dataset_type'] in ['val', 'all']:
